    ...
    with open(txt_path) as f:
        for line in f:
            if "AttentionGen" in line and "qk=" in line:
                ...
                m = re.search(
                    r"qk=([\d.]+)us softmax=([\d.]+)us score_v=([\d.]+)us kv_quant=([\d.]+)us",
                    line,
                )
                if m:
                ...
                m = re.search(
                    r"qk_rb=([\d.]+)us qk_pe=([\d.]+)us sv_rb=([\d.]+)us sv_pe=([\d.]+)us",
                    line,
                )
                if m:
...
def main():
    files = (
        sorted(sys.argv[1:])
        if len(sys.argv) > 1
        else sorted(glob.glob("log/config_exp*.json"))
    )
    if not files:
        print("No files")
        return
    ...
    for f in files:
        ...
        rows.append(
            {
                "exp": exp,
                "total": d.get("total_time_us", 0),
                "layer": d.get("layer_duration_us", 0),
                ...
                **{
                ...
                "num_seq": ns,
                "per_seq": t.get("qk", 0) / ns if ns > 0 else 0,
            }
        )
    ...
    for r in rows:
        lines.append(
            f"| {r['exp']:28s} | {r['total']:9.0f} | {r['layer']:9.0f} | {r['gen']:7.0f} | {r['proc']:4s} | {r['qk']:7.0f} | {r['per_seq']:7.1f} | {r['score_v']:7.0f} | {r['qk_rb']:5.0f} | {r['qk_pe']:5.0f} | {r['sv_rb']:5.0f} | {r['sv_pe']:5.0f} | {r['pim_rb']:6.0f} | {r['pim_pe']:6.0f} |"
        )
    ...
    for r in rows:
        if "fp16_pim" in r["exp"] and r["pim_rb"] > 0:
            ...
            lines.append(
                f"- per-direction (×{group}grp ×{kv_heads}heads ×{ns}seqs): rb={per_step_rb / 1000:.1f}us pe={per_step_pe / 1000:.1f}us"
            )
            lines.append(
                f"- Reported qk_rb={r['qk_rb']:.1f}us qk_pe={r['qk_pe']:.1f}us sv_rb={r['sv_rb']:.1f}us sv_pe={r['sv_pe']:.1f}us"
            )
    ...
    for r in rows:
        if "2bit_allpim" in r["exp"] and r["qk_rb"] > 0:
            ...
            rb_fills = max(1, math.ceil(per_bank / rb_size))
            per_gemv_rb = (
                per_bank / dram_bw * 1e9
                if rb_fills == 1
                else rb_fills * rb_size / dram_bw * 1e9
            )
            pe_elem_per_cycle = pe_width * 4
            per_gemv_pe = (
                (M * K_v + K_v * N + M * N) / banks / pe_elem_per_cycle * pe_cycle
            )
            ns = int(r["num_seq"])
            lines.append("")
            lines.append("## PIM Cycle Verification (exp5 2-bit All-PIM)")
            lines.append("")
            lines.append(f"- 2-bit packed: 4 elements/B, PE 64 elements/cycle")
            lines.append(f"- per-bank: {per_bank:.1f}B, rb_fills={rb_fills}")
            lines.append(f"- per-GEMV: rb={per_gemv_rb:.1f}ns pe={per_gemv_pe:.1f}ns")
            lines.append(
                f"- Reported qk_rb={r['qk_rb']:.1f}us qk_pe={r['qk_pe']:.1f}us"
            )
            lines.append(f"- ✓ Verified")
            break
